...
# 加载数据集
data = datasets.ImageFolder(root=r'D:\6666\CVT\allin_file',transform=tf)
# data = datasets.ImageFolder(root=r'D:\6666\CVT\AugmentedAlzheimerDataset',transform=tf)

train_size = int(0.7 * len(data))  # 70% 用于训练
val_size = int(0.2 * len(data))  # 剩余的 20% 用于验证
test_size = len(data) - train_size - val_size
# train_size = int(0.8 * len(data))  # 80% 用于训练
# val_size = len(data) - train_size  # 剩余的 20% 用于验证
# 将数据集分为训练集和验证集
...
for epoch in range(200):
    ...
    with torch.no_grad():
        ...
        # 如果当前准确率高于历史最高准确率，则保存模型
        if acc > best_acc:
            best_acc = acc
            # torch.save(model.state_dict(), 'allin_model.pth')
