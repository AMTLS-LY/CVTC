...
class CrossFormer(nn.Module):
    ...
    def forward(self, x):
        for i, (cel, transformer) in enumerate(self.layers):
            x = cel(x)
            #torchvision.utils.save_image(x, f'output_after_cel_layer_{i}.png')
            #print(f'After CrossEmbedLayer: {x.size()}')
            x = transformer(x)
            #torchvision.utils.save_image(x, f'output_after_transformer_layer_{i}.png')
...
# 加载状态字典
state_dict = torch.load(r"E:\dataset\best_model.pth")
...
dataset=datasets.ImageFolder(root=r"E:\dataset\AugmentedAlzheimerDataset",transform=tf)
# 定义交叉验证策略 
kf = KFold(n_splits=5)
...
for train_index, val_index in kf.split(dataset):
    ...
    with torch.no_grad():
        for inputs, labels in val_loader:
            ...
            outputs = model(inputs).float()  # 确保输出是浮点类型
    ...
    fold_performance.append((val_loss / len(val_loader), accuracy))

# 计算平均性能
average_loss = sum([x[0] for x in fold_performance]) / len(fold_performance)
average_accuracy = sum([x[1] for x in fold_performance]) / len(fold_performance)
